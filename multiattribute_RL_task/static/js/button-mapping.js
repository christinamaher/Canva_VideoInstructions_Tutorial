jsPsych.plugins['button-mapping'] = (function() {
...
//      iti_duration: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var response = {
      ...
      trial_onset: null,
      trial_onset_PN: null,
      feedback_onset: null, // this is when the participant sees the outcome of their selection
      iti_onset: null,
      decision_onset_PN: null,
    };
	...
	var photodiode_square = trial.photodiode_paths[1]; // use a white square so we don't cue photodiode deflections
   ...
   // display.photodiode_path = trial.photodiode_paths[0];
    ...
    function DrawBackground(){
      ...
      ctx.fillStyle = "grey";
      ctx.textAlign = "center";
      var info_text_1 = "Practice using the arrow keys on the keyboard to pick a gem."
      var info_text_2 = "LEFT, DOWN, RIGHT"
    ...
    response.trial_onset_PN = performance.now();
      ...
      if (display.center_box == "selected"){
        _DrawSelectionBox(0, trial.stimulus_offset[1], trial.selection_colour);_DrawPhotodiodeStimulus(photodiode_square, [trial.photodiode_offset[0], trial.photodiode_offset[1]]);
    ...
    function AfterResponse(info) {
     ...
     if (response.key_char == trial.left_key){
      ...
      counter.button_mapping_rewards = counter.button_mapping_rewards + response.feedback;
 ...
 // set a timeout to end the ITI after a given delay
    ...
    // draw the background of the canvas
      ...
      var trial_data = {
        ...
        'rt': response.rt,
        'decision_onset': response.decision_onset,
        'decision_onset_PN': response.decision_onset_PN,
        'feedback_onset': response.feedback_onset,
        'trial_onset': response.trial_onset,
        'trial_onset_PN': response.trial_onset_PN,
        ...
        'ITI_onset': response.iti_onset, 
        'ITI_length': response.iti_length,    
      };
    ...
    function _DrawPhotodiodeStimulus(photodiode_stimulus, photodiode_stimulus_offset){
        ...
        // specify that the image should be drawn once it is loaded
        photodiode_img.onload = function(){_PhotodiodeOnload(photodiode_img, trial.photodiode_dimensions, trial.photodiode_offset)};
        ...
        function _ClearPhotodiodeStimulus(clear_photodiode_stimulus, clear_photodiode_stimulus_offset){
  
        // create new image element
        var photodiode_clear_img = new Image();

        // specify that the image should be drawn once it is loaded
        photodiode_clear_img.onload = function(){_PhotodiodeOnload(photodiode_clear_img, trial.photodiode_dimensions, trial.photodiode_offset)};

        // set the source path of the image; in JavaScript, this command also triggers the loading of the image
        photodiode_img.src = photodiode_clear;
    ...
    function _ImageOnload(im, image_dimensions, image_offset){
      ...
      ctx.drawImage(im, stim_horiz_loc, stim_vert_loc, image_dimensions[0], image_dimensions[1]);
        ...
        function _PhotodiodeOnload(p_im, p_image_dimensions, p_image_offset){

      var p_stim_horiz_loc = (trial.canvas_dimensions[0]/2) - p_image_offset[0]  + (p_image_dimensions[0] / 2); // specifies the x coordinate of the top left corner of the stimulus
      var p_stim_vert_loc = (trial.canvas_dimensions[1]/2) + p_image_offset[1] - (p_image_dimensions[1] / 2); // specifies the y coordinate of the top left corner of the stimulus
      ctx.drawImage(p_im, p_stim_horiz_loc, p_stim_vert_loc, p_image_dimensions[0], p_image_dimensions[1]);
